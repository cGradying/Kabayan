...
Rules: only set a slot if the message (or already-known state) actually supports it — never guess. "evidence" is a short paraphrase of what THIS message adds. "picked_source_id" is only set if the user is clearly choosing one of the options already shown (match by id). Keep intent from known state if this message doesn't change it — UNLESS this message is off_topic, which always overrides (even a known intent) for that turn.
